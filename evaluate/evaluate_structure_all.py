...
if __name__ == "__main__":
    ...
    print(len(all_items))
    ty = 'outline_base'
    with open('../results/qwen_outline_base_0913.json') as fp:
        data = json.load(fp)
    # for item in data.keys():
    print(ty)
    for item in all_items[100:300]:
    # for item in all_items:
    # try:
        print(item)
        folder_path = f'../writing_agents_results_gpt/{ty}/{item}/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        save_file_path = f'../writing_agents_results_gpt/{ty}/{item}/introduction_results.json'
        if not os.path.exists(save_file_path): 
            sa = get_parts(data[item]['introduction'])
        ...
        file_path = f'../writing_agents_results_gpt/{ty}_judge/{item}.json'
    ...
    # nohup python evaluate_structure_all.py > gpt_naive_judge_600.log 2>&1 &