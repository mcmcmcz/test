...
def main():
    ...
    for item in tqdm(list(all_data.keys())[600:]):
        paper_id = item
        if not os.path.exists(f"writing_agents_results/outline_ta/{paper_id}/introduction_results.json"):     
            folder_path = f"writing_agents_results/outline_ta/{paper_id}/"
            ...
            result = generator.generate_from_data(
                ...
                target_length=700,
                save_path=f"writing_agents_results/outline_ta/{paper_id}/introduction_results.json"
            )
