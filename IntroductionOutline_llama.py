...
GLOBAL_MODEL = None
GLOBAL_TERMINATORS = None


def load_model(model_path):
    """加载开源模型和tokenizer"""
    global GLOBAL_TOKENIZER, GLOBAL_MODEL, GLOBAL_TERMINATORS
    ...
    GLOBAL_MODEL = model
    GLOBAL_TERMINATORS = terminators

    return tokenizer, model, terminators

GLOBAL_TOKENIZER, GLOBAL_MODEL, GLOBAL_TERMINATORS = load_model("/home/mczhang/zmc-dl/LLM/Meta-Llama-3.1-8B-Instruct")
...
def model_generate(messages):
    ...
    generated_ids = GLOBAL_MODEL.generate(
        **model_inputs,
        ...
        eos_token_id=GLOBAL_TERMINATORS)
    generated_ids = [
...
def main():
    ...
    for item in all_data.keys():
        paper_id = item
        if not os.path.exists(f"writing_agents_results_llama/outline/{paper_id}/introduction_results.json"):     
            folder_path = f"writing_agents_results_llama/outline/{paper_id}/"
            ...
            result = generator.generate_from_data(
                ...
                target_length=700,
                save_path=f"writing_agents_results/outline_llama/{paper_id}/introduction_results.json"
            )
