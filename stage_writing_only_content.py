...
def transfer(temp, path):
    ...
    outlines = {}
    o = []
    start = temp.index('<STAGE0>')
    end = temp.index('<STAGE1>')
    ...
    start = temp.index('<STAGE2>')
    end = temp.index('<STAGE3>')
...
MODEL_PATH = "/home/mczhang/zmc-dl/LLM/LLaMA-Factory/qwen_stage_content_only_1123"
...
def main():
    ...
    for item in tqdm(list(all_data.items())):
        ...
        folder_path = f"writing_agents_results/stage_content_only/{paper}/"
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        file_path = f"writing_agents_results/stage_content_only/{paper}/introduction_results.json"
        if not os.path.exists(file_path): 
            ...
            When writing, ensure logical coherence and smooth transitions, and use fluent and standard academic English.""",
                ...
                {
                    "role":"user",
                    "content": f"""Compose the Introduction of an ACL paper based on the corresponding research materials. You need to write four sections:
            ...
            special_tokens = [f'<STAGE{i}>' for i in range(4)] + [f'<END{i}>' for i in range(4)] + ["Contents for " + section for section in sections]
            for i in range(3):
                judge = True
