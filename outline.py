...
def load_model(model_path):
    ...
    model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.bfloat16, device_map="sequential")
    terminators = [
...
def model_generate(tokenizer, model, terminators, messages, if_eval=False, return_logits=False):
    ...
    if return_logits:
        ...
        token_probs = torch.tensor([probs[i][0, token_id].item() for i,token_id in enumerate(generated_ids[0][len(input_ids[0]):])])
        certainty_likelihood = token_probs.prod().pow(1/len(token_probs)).item()
        certainty_mean = torch.mean(token_probs).item()
        certainty = [certainty_likelihood,certainty_mean]
...
class Agent:
    """基础智能体类"""
    def __init__(self, name, system_prompt, use_closeai, model, tokenizer, terminators):
        self.name = name
        ...
        if use_closeai:
            ...
            self.tokenizer, self.terminators, self.model = tokenizer, terminators, model
    ...
    def generate(self, prompt: str) -> str:
        ...
        if self.use_closeai:
            ...
            return model_generate(self.tokenizer, self.model, self.terminators, messages)
...
class OutlineAgent(Agent):
    """大纲生成智能体"""
    def __init__(self, use_closeai, model, tokenizer, terminators):
        super().__init__(
            ...
            model=model, tokenizer=tokenizer,terminators=terminators
        )
...
class OutlineSystem:
    """大纲生成系统"""
    def __init__(self, use_closeai, model, tokenizer, terminators):
        self.outline_agent = OutlineAgent(use_closeai, model, tokenizer, terminators)
        self.introduction_writer = IntroductionWriter(use_closeai, model, tokenizer, terminators)
...
class IntroductionWriter(Agent):
    ...
    def __init__(self, use_closeai, model, tokenizer, terminators):
        super().__init__(
            ...
            4. Focus on the content without referencing other works
...
def main():
    ...
    introduction_system = OutlineSystem(use_closeai, model, tokenizer, terminators)
    ...
    with open('results/llama_outline_base_0913.json', 'w', encoding='utf-8') as fp:
